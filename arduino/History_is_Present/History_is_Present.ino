...
void fillMask(uint8_t* mask, uint8_t v);
void fillMask(uint8_t* mask, uint8_t v, int start, int stop);
void apply_mask(uint8_t *mask);
void apply_mask(uint8_t* mask, CRGB color);
...
void SolidColor_display_time(uint32_t last_tm, uint32_t next_tm);
void Cloud_init();
void Cloud_display_time(uint32_t last_tm, uint32_t next_tm);
void Fire_init();
void Fire_display_time(uint32_t last_tm, uint32_t next_tm);
void Glitch_init();
void Glitch_display_time(uint32_t last_tm, uint32_t next_tm);
uint32_t Now();
...
void setPixelMask(uint8_t* mask, uint8_t row, uint8_t col, uint8_t v){
  if(row >= MatrixHeight){
  }
...
void wipe_around(bool val){
  ...
  while (theta < 3.14 + dtheta){
    ...
    if(val){
      logical_or(NUM_LEDS, (bool*)wipe, (bool*)mask, (bool*)tmp);
    }
    else{
      logical_and(NUM_LEDS, (bool*)wipe, (bool*)mask, (bool*)tmp);
    }
    apply_mask(tmp);
    my_show();
    theta += dtheta;
...
void word_fade_in(uint16_t time_inc){
  ...
  for(uint8_t j = 0; j < n_byte_per_display; j++){ // j is a byte index 
	...
	tmp_word[1] = word[1] + 1;
	tmp_word[2] = word[2];
	if(word[1] % 2  == 0){ // do both rows at the same time
	  setWordMask(mask, word, 1);
	  setWordMask(mask, tmp_word, 1);
...
void Plain_drop(uint16_t last_tm_inc, uint16_t time_inc){
  word_fade_in(time_inc);
}
void Plain_init(){
  return;
}
void Plain_display_time(uint32_t last_tm, uint32_t tm){
  ...
  if(last_tm_inc != tm_inc || (last_tm_inc == 287 && tm_inc == 0)){
    FadeToBlack();
    for(int ii=0; ii < 20; ii++){
      my_show();
      delay(100);
    }
    Plain_drop(last_tm_inc, tm_inc);
  }
...
void setWordMask(uint8_t *mask, uint8_t* word, uint8_t v){
  // word = [row, col, len]
  for(int i=0; i < word[2]; i++){
    setPixelMask(mask, word[1], word[0] + i, v);
  }
}
...
void word_drop_in(uint16_t time_inc){
  ...
  for(uint8_t j = 0; j < n_byte_per_display; j++){ // j is a byte index 
	...
	for(int rr = 0; rr <= word[1]; rr++){
	  ...
	  logical_or(NUM_LEDS, (bool*)mask, (bool*)wipe, (bool*)tmp_mask);
	  rainbow_slow();
	  apply_mask(tmp_mask);
	  my_show();
	  delay(25);
...
void word_drop_out(uint16_t time_inc){
  ...
  //fillMask(wipe, false);
  //fillMask(tmp_mask, false);
  logical_copy(NUM_LEDS, (bool*)mask, (bool*)wipe);
  logical_copy(NUM_LEDS, (bool*)mask, (bool*)tmp_mask);
  ...
  for(uint8_t j = 0; j < n_byte_per_display; j++){ // j is a byte index 
	...
	tmp_word[1] = word[1];
	tmp_word[2] = word[2];
	for(int rr = word[1]; rr <= MatrixHeight; rr++){
	  tmp_word[1] = rr;
	  setWordMask(wipe, tmp_word, true);
	  logical_or(NUM_LEDS, (bool*)mask, (bool*)wipe, (bool*)tmp_mask);
	  rainbow_slow();
...
void TheMatrix_drop(uint32_t last_tm_inc, uint32_t current_tm_inc){
  ...
  while(!logical_equal(NUM_LEDS, (bool*)wipe, (bool*)have)){
    //  while(millis() < end){
    fadeToBlackBy(leds, NUM_LEDS, 75);
    for(i = 0; i < n_drop; i++){
      if(millis() > end && wipe[XY(cols[i], rows[i])]){
	if(random(0, 1) == 0){
	  have[XY(cols[i], rows[i])] = 255;
	}
      }
  ...
  for(int ii=0; ii< MatrixHeight * 5; ii++){
    //  while(millis() < end){
    fadeToBlackBy(leds, NUM_LEDS, 75);
    for(i = 0; i < n_drop; i++){
      rows[i]++;
...
void SolidColor_display_time(uint32_t last_tm, uint32_t tm){
  last_tm *= 3;
  tm *= 3;
  if(last_tm/300 != tm/300 || force_update){
    force_update = false;

    FadeToBlack();
    for(int ii=0; ii < 20; ii++){ // pause while waiting for user inputs
      my_show();
      delay(100);
    }

    // set new time display
    fillMask(mask, 0);
    Faceplates[config.faceplate_idx % N_FACEPLATE].maskTime(tm, mask);
    // reset masked values to dim (1)
    for(int ii=0; ii<NUM_LEDS; ii++){
      if(mask[ii] > 0){
	mask[ii] = 1;
      }
...
void Cloud_transition(uint32_t last_tm, uint32_t tm){
  ...
  for(int mask_to=0; mask_to<MatrixWidth; mask_to++){
    for(int row=0; row < MatrixHeight; row++){
      setPixelMask(mask, row, mask_to, 255);
    }
  ...
  fillMask(mask, 255);
  // bring fire out!
  for(int fill_to=0; fill_to < MatrixWidth; fill_to++){
    for(int row=0; row < MatrixHeight; row++){
...
void Fire_transition(uint32_t last_tm, uint32_t tm){
  ...
  fillMask(mask, 0);
  Faceplates[config.faceplate_idx % N_FACEPLATE].maskTime(last_tm, mask);
  for(int mask_to=MatrixHeight-1; mask_to>=0; mask_to--){
    for(int col=0; col < MatrixWidth; col++){
      setPixelMask(mask, mask_to, col, 255);
    }
    for(int ii=0; ii<1; ii++){
      fireNoise2();
      apply_mask(mask);
  ...
  fillMask(mask, 255);
  // bring fire out!
  for(int fill_to=MatrixHeight-1; fill_to>=0; fill_to--){
    for(int col=0; col < MatrixWidth; col++){
...
void Glitch_transition(uint32_t last_tm, uint32_t tm){
  FadeToBlack();
  fillMask(mask, 0);
  for(int ii=0; ii < 20; ii++){ // pause while checking for user inputs
    my_show();
    delay(100);
  }
  Faceplates[config.faceplate_idx % N_FACEPLATE].maskTime(tm, mask);
...
void display_bitmap_rgb(uint8_t* bitmap){
  uint8_t n = 16;
  uint8_t h = MatrixHeight;
  uint8_t w = MatrixWidth;
  uint8_t xy[2];
  uint8_t r, g, b;
  ...
  for(i=0; i<n; i++){
    ...
    // 7 eeeeeeeeffffffff
    for(j=0; j<MatrixHeight; j++){
      x = (i * MatrixHeight + j) % MatrixWidth;
      y = i / 2;
      led_idx = XY(x, y);
...
uint16_t XY( uint8_t x, uint8_t y){
  ...
  /*
  Serial.print("XY:x:"); Serial.println(x);
  Serial.print("XY:y:"); Serial.println(y);
  Serial.print("XY:out:"); Serial.println(out);
  */
  return out;
}
...
void wifi_setup(){
  if(config.wifi_reset){
    ...
    saveSettings();
    wifiManager.startConfigPortal("History_is_Present");
  }
  else{
    wifiManager.autoConnect("History_is_Present");
  }
...
uint32_t random_start_time;
void setup(){
  random_start_time = random(0, 86400);
  last_time = 0;
  delay(6000); // pause to check for programs to upload
  ...
  Serial.print("Matrix Width:");Serial.println(MatrixWidth);
  Serial.print("Matrix Height:");Serial.println(MatrixHeight);
...
uint32_t Now(){
  uint32_t out;
  return millis()/100 + random_start_time;
}
